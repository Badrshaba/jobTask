...
export default async function ProductDetails({ params }: { params: Promise<{ _id: string }> }) {
  ...
  return (
    <section className='flex  justify-center items-center gap-2 min-h-[90vh] '>
      <Suspense fallback={<ProductDetailsLoading />}>
        <div className='md:w-9/12 w-11/12 mx-auto border-2 min-h-[50%] my-10  lg:h-[60vh] flex flex-col lg:flex-row justify-center items-center gap-2 '>
          <Image
            src={product.image}
            alt={product.title}
            width={300}
          ...
          />
          <div className='lg:w-6/12 w-full flex lg:h-10/12 flex-col space-y-2 justify-around gap-2'>
